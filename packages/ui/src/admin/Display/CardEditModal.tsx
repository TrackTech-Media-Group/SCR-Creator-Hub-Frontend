...
import { DangerButton, PrimaryButton } from "@creatorhub/buttons";
import { PulseLoader } from "react-spinners";
import { FileEntry } from "../FileEntry";
import Dropzone from "react-dropzone";
import { v4 } from "uuid";
import { PartialFileEntry } from "./PartialFileEntry";
...
export const CardEditModal: React.FC<Props> = ({ isOpen, onClick, onSubmit, tags, data }) => {
	const validation = object({
		...
		useCases: string(),
		downloads: array(object({ dimensions: string().required(), type: string().required().oneOf(["HD", "QHD"]) }))
	});
		...
		<Modal isOpen={isOpen} onClick={onClick}>
			...
			<Formik
				...
				initialValues={{
					...
					existingDownloads: data.downloads.map((v) => ({ id: v.id, name: v.name })) as { id: string; name: string }[],
							...
							{formik.values.existingDownloads.map((value, key) => (
								<PartialFileEntry
									...
									removeItem={(id) =>
										formik.setFieldValue(
											"existingDownloads",
											formik.values.existingDownloads.filter((v) => v.id !== id)
										)
									}
								/>
							))}
							<p className="text-red-500 text-base">
								{formik.errors.existingDownloads &&
									`* ${typeof formik.errors.existingDownloads === "string" ? formik.errors.existingDownloads : ""}`}
								&#8203;
							</p>
						</div>
